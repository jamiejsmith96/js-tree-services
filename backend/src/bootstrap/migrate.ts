...
// Define minimal Strapi interface for our needs
...
async function uploadFile(strapi: StrapiInterface, filePath: string, name: string) {
  try {
...
async function migrateBlogPosts(strapi: StrapiInterface) {
  const blogPosts = [
    {
...
async function migrateGalleryItems(strapi: StrapiInterface) {
  const galleryItems = [
    {
...
export default {
  async migrate(strapi: StrapiInterface) {
    try {
